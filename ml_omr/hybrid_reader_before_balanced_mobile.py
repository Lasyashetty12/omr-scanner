...
# The option letter itself can make the small centre/core masks look dark.
# A genuinely filled bubble darkens almost the entire printed disk.  Use this
# wider mask as the primary guard against false MULTIPLE/UNCERTAIN results.
SOLID_FILL_DISK_RATIO = 0.82
DOMINANT_DISK_RATIO = 0.72
DOMINANT_DISK_GAP = 0.16
...
def _bubble_metrics(crop):
    ...
    raw_disk_pixels = raw_square[disk_mask]
    ...
    # Preserve absolute ink density before CLAHE. CLAHE is useful for faint
    # marks, but on blurred photos it can over-amplify the printed option
    # letter in an empty bubble. A genuinely filled bubble has a much lower
    # raw disk mean across the complete interior.
    raw_disk_mean = float(
        np.mean(
            raw_disk_pixels
        )
...
def _decide_question(
    ...
    raw_option, raw_mean, raw_dark_ratio = raw_ranked[0]
    second_raw_mean = raw_ranked[1][1]

    # Primary decision for plainly filled bubbles. Requiring both a dark
    # absolute disk and a large within-row gap makes this insensitive to page
    # shadows while preventing the printed A/B/C/D letters from winning.
    if (
        raw_mean <= 122.0
        and raw_dark_ratio >= 0.25
        and second_raw_mean - raw_mean >= 10.0
    ):
        return {
            "answer": raw_option,
            "status": "answered",
            ...
            "best_darkness": round(best_darkness, 3),
            "second_darkness": round(second_darkness, 3),
            "top_gap": round(top_gap, 3),
            "question_blank_baseline": round(
                question_blank_baseline,
                3,
            ),
            "best_delta": round(best_delta, 3),
            "raw_disk_fill": True,
            "raw_disk_mean": round(raw_mean, 2),
            "second_raw_disk_mean": round(second_raw_mean, 2),
        }
    ...
    # A solid disk is much stronger evidence than the darkness of the printed
    # A/B/C/D character. Resolve these marks before the legacy core-relative
    # rules below. This also handles vertically touching ink correctly because
    # each option is still evaluated inside its own fitted bubble disk.
    if len(solid_fill_options) == 1:
        solid_option = solid_fill_options[0]

        return {
            "answer": solid_option,
            "status": "answered",
            "multiple_options": [],
            "best_option": solid_option,
            "best_darkness": round(best_darkness, 3),
            "second_darkness": round(second_darkness, 3),
            "top_gap": round(top_gap, 3),
            "question_blank_baseline": round(
            ...
            "best_delta": round(best_delta, 3),
            "solid_disk_fill": True,
        }

    if len(solid_fill_options) >= 2:
        return {
            "answer": "MULTIPLE",
            "status": "multiple",
            "multiple_options": solid_fill_options,
    ...
    # Mobile blur can join consecutive filled bubbles vertically. In that
    # situation the fitted centre is still correct, but the wider disk ratio
    # may land just below the strict solid threshold. Accept it only when one
    # disk is both substantially filled and clearly separated from every
    # other option in the same row. The separation guard preserves blanks and
    # real multiple marks.
    dominant_option, dominant_ratio = disk_ranked[0]
    second_disk_ratio = disk_ranked[1][1]

    if (
        dominant_ratio >= DOMINANT_DISK_RATIO
        and dominant_ratio - second_disk_ratio >= DOMINANT_DISK_GAP
    ):
        return {
            "answer": dominant_option,
            "status": "answered",
            "multiple_options": [],
            "best_option": dominant_option,
            "best_darkness": round(best_darkness, 3),
            "second_darkness": round(second_darkness, 3),
            "top_gap": round(top_gap, 3),
            "question_blank_baseline": round(
            ...
            "best_delta": round(best_delta, 3),
            "dominant_disk_fill": True,
            "dominant_disk_ratio": round(dominant_ratio, 4),
            "second_disk_ratio": round(second_disk_ratio, 4),
        }
    ...
    # Final conservative single-mark fallback for uneven mobile lighting.
    # Both independent measurements must select the same option, and the raw
    # bubble disk must still be materially darker than the runner-up. This
    # resolves obvious filled bubbles that miss the stronger thresholds while
    # preserving genuinely blank/unclear rows as UNCERTAIN.
    if (
        raw_option == best_option
        and raw_mean <= 125.0
        and raw_dark_ratio >= 0.25
        and second_raw_mean - raw_mean >= 10.0
        and best_darkness >= 55.0
    ):
        return {
            "answer": raw_option,
            "status": "answered",
