...
def refine_bubble_center(
    ...
    """
    Tiny final per-bubble recentering step.

    The column calibration already removes the large/gradual mobile-photo
    distortion. This only searches a few pixels around the calibrated point
    so the crop is centered on the printed bubble ring.
    """
    ...
    # Precompute search candidates in a tiny +/-3 px window.
    for dy in range(
        -search_radius,
        search_radius + 1,
    ):
        for dx in range(
            ...
            # Printed ring should be darker than nearby paper.
            score = (
                outer_mean
                -
                ring_mean
